...
class BingoStatistics:
    ...
    def calcu(self,input:[]):
        ...
        setConcat = list(map(lambda r:set(r['balls'] + input),self.bingoData))
        arrConcat = list(map(lambda r:list(r),setConcat))
        arrTermConcat = list(map(lambda x,y:{'term':y['term'],'balls':x},arrConcat,self.bingoData))
        arrFilter = list(filter(lambda r:len(r['balls']) == self.BALL_NUM,arrTermConcat))
